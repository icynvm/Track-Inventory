    ...
    <div className="w-full max-w-md space-y-8 animate-in fade-in slide-in-from-bottom-10 duration-700">
      <div className="text-center space-y-4">
        ...
        <div className="space-y-1">
          ...
          <p className="text-slate-400 text-sm font-bold uppercase tracking-[0.2em]">Google Sheet Synced Terminal</p>
        </div>
      </div>
      <div className="space-y-4">
        <button onClick={() => onLogin('user')} className="group relative w-full p-8 bg-white border-2 border-slate-100 rounded-[2.5rem] shadow-sm hover:border-blue-500 hover:shadow-xl transition-all duration-300 flex flex-col items-center text-center gap-4">
          ...
          <div><span className="block text-lg font-black text-slate-900 tracking-tight">Production Crew</span><span className="block text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Check Assets</span></div>
        </button>
        <button onClick={() => onLogin('admin')} className="group relative w-full p-8 bg-white border-2 border-slate-100 rounded-[2.5rem] shadow-sm hover:border-slate-900 hover:shadow-xl transition-all duration-300 flex flex-col items-center text-center gap-4">
          ...
          <div><span className="block text-lg font-black text-slate-900 tracking-tight">Asset Manager</span><span className="block text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Edit Master List</span></div>
        </button>
      </div>
    </div>
  </div>
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-1">
        <div className="space-y-2">
          <div className="flex items-center gap-3"><span className="px-3 py-1 bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest rounded-lg">{role} mode</span></div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-10 pb-24 md:pb-12">
        ...
        <div className="bg-white rounded-[3rem] shadow-sm border border-slate-100 flex flex-col h-[600px] md:h-[800px] sticky top-28">
          ...
          <div className="p-8 space-y-8 overflow-y-auto custom-scrollbar">{logs.length === 0 ? <div className="text-center py-32 text-slate-300 italic flex flex-col items-center gap-6"><svg className="w-20 h-20 text-slate-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg><p className="text-xs font-black uppercase tracking-[0.3em]">No Transmission Data</p></div> : logs.map((log:any) => (
            <div key={log.id} className="flex gap-5 group">
              <div className={`mt-1 flex-shrink-0 w-12 h-12 rounded-[1.25rem] flex items-center justify-center transition-all group-hover:scale-110 shadow-lg ${log.action === 'CHECK_OUT' ? 'bg-blue-600 text-white shadow-blue-100' : 'bg-slate-900 text-white shadow-slate-100'}`}>{log.action === 'CHECK_OUT' ? <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M17 16l4-4m0 0l-4-4m4 4H7" /></svg> : <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M11 16l-4-4m0 0l4-4m-4 4h14" /></svg>}</div>
              <div className="flex-1 overflow-hidden">
                <p className="text-sm leading-tight mb-2"><span className="font-black text-slate-900 uppercase tracking-tight">{log.userName}</span><span className="text-slate-400 font-bold"> {log.action === 'CHECK_OUT' ? 'deployed' : 'restored'} </span><span className="font-black text-blue-600 block truncate">{log.equipmentName}</span></p>
                {log.projectName && <div className="inline-flex items-center gap-2 bg-slate-50 text-slate-500 px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border border-slate-100"><svg className="w-3 h-3 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M7 7h.01M7 3h5l7 7-7 7H7a4 4 0 01-4-4V7a4 4 0 014-4z" /></svg>{log.projectName}</div>}
                <p className="text-[9px] text-slate-300 mt-3 uppercase font-black tracking-widest">{new Date(log.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} • {new Date(log.timestamp).toLocaleDateString()}</p>
              </div>
            </div>
          ))}</div>
        </div>
...
const ScanPage: React.FC<any> = ({ onScan }) => (
  <div className="flex flex-col items-center justify-center min-h-[80vh] space-y-12 animate-in fade-in duration-700 px-4">
    <div className="text-center max-w-lg space-y-4">
      <div className="mx-auto w-16 h-16 bg-blue-600 text-white rounded-[1.5rem] flex items-center justify-center shadow-2xl shadow-blue-200"><svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01" /></svg></div>
    ...
    <Link to="/" className="text-slate-400 hover:text-slate-900 flex items-center gap-3 transition-colors font-black text-xs uppercase tracking-[0.2em] bg-white px-6 py-3 rounded-2xl shadow-sm border border-slate-100"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>Abort Mission</Link>
  </div>
);
